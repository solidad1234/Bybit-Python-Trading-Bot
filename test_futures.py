...
def fetch_candles(symbol, interval, limit=50):
    try:
        ...
        if data.get("retCode") == 0:
            ...
            print(f"Error in fetch_candles: {data.get('retMsg')}")
...
def fetch_order_book(symbol):
    try:
        ...
        if data.get("retCode") == 0:
            ...
            print(f"Error in fetch_order_book: {data.get('retMsg')}")
    except Exception as e:
        print(f"Exception in fetch_order_book: {e}")
    return None
...
def analyze_and_trade():
    ...
    if not order_book:
        print("Order book data unavailable. Retrying...")
        return
